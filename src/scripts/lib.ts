...
function previewIframeHTML(r: ResultView, resultKey: string): string {
  ...
  // Store the demo in a <textarea>, not <script>: script text via innerHTML
  // keeps entities literal (&lt;…), so postMessage would send escaped junk.
  // Textarea value decodes entities back to the real HTML document.
  return `
    <div class="relative h-full bg-[var(--color-surface)] p-1.5">
      ...
      <textarea hidden data-preview-source>${esc(doc)}</textarea>
...
function readPreviewSource(frame: HTMLIFrameElement): string | null {
  ...
  if (holder instanceof HTMLTextAreaElement) return holder.value;
